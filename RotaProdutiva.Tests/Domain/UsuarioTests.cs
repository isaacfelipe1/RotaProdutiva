...
namespace RotaProdutiva.Tests.Domain
{
    public class UsuarioTests
    {
        ...
        {
            var usuario = new Usuario("João Silva", "Joao@Example.com", "hash123", "11999999999", TipoUsuario.Jovem);

            Assert.Equal("João Silva", usuario.Nome);
            Assert.Equal("joao@example.com", usuario.Email);
            Assert.True(usuario.EhJovem());
        ...
        public void Deve_Lancar_Excecao_Quando_Nome_Vazio()
        {
            Assert.Throws<ArgumentException>(() =>
                new Usuario("", "joao@example.com", "hash123", "11999999999", TipoUsuario.Jovem));
        }
        ...
        {
            Assert.Throws<ArgumentException>(() =>
                new Usuario("João Silva", "", "hash123", "11999999999", TipoUsuario.Jovem));
        }
        ...
        {
            Assert.Throws<ArgumentException>(() =>
                new Usuario("João Silva", "joao@example.com", "", "11999999999", TipoUsuario.Jovem));
        }
